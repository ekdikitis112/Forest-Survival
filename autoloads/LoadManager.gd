...
func load_scene(scene_path: String):
	...
	get_tree().get_root().get_node("Main").add_child(new_loading_screen)
...
func _process(_delta):
	...
	match load_status:
		...
		3: #? THREAD_LOAD_LOADED
			...
			print(_loaded_resource)
			get_tree().get_root().get_node("Main").add_child(_loaded_resource.instantiate())
